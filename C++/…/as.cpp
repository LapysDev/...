...
/* ... */
struct as{template<class T>struct p{T&&x;template<class U>[[nodiscard]]constexpr auto operator->*(U*)noexcept(noexcept(U((T&&)x))){return U((T&&)x);}};explicit as()=default;friend constexpr auto operator->*(auto&&x,as)noexcept{return p((decltype(x)&&)x);}};constexpr void*operator new(decltype(0uz),as)noexcept{return 0;}
#define as ->*::as()->*::new(::as())
